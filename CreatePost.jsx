...
const CreatePost = () => {
  ...
  const [common, setCommon] = useState({
    ...
    title: '',
    description: '',
    media: null, // Görsel upload placeholder
    startDate: '',
    endDate: '',
  });
  ...
  const [google, setGoogle] = useState({
    enabled: true,
    type: 'UPDATE', // UPDATE | EVENT | OFFER
    actionType: 'LEARN_MORE',
    actionUrl: '',
    offer: { couponCode: '', redeemUrl: '', terms: '' }
  });

  // Apple Business Connect Config
  const [apple, setApple] = useState({
    enabled: false,
    actionType: 'visit_website', // none | visit_website | call | directions | book
    actionUrl: ''
  });

  // --- HANDLERS ---

  const handleGoogleTypeChange = (e) => {
    setGoogle({ ...google, type: e.target.value });
  };

  const handleAppleActionChange = (e) => {
    // Apple Logic: Eğer 'visit_website' değilse, URL input'u sıfırla veya pasife çek
