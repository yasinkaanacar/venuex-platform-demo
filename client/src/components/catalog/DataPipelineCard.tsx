...
function getPlatformMeta(platform: 'google' | 'meta') {
  if (platform === 'google') {
    return { name: 'Google Merchant Center', bgColor: 'bg-[#4285F4]', letter: 'G' };
  }
  return { name: 'Meta Commerce', bgColor: 'bg-[#1877F2]', letter: 'M' };
}

function getSyncBadge(status: PlatformSyncStatus['syncStatus'], oc: any) {
  switch (status) {
    case 'healthy':
      return { label: oc?.healthy || 'Healthy', className: 'bg-green-50 text-green-700 border-green-200' };
    case 'warning':
      return { label: oc?.warning || 'Warning', className: 'bg-amber-50 text-amber-700 border-amber-200' };
    case 'error':
      return { label: oc?.error || 'Error', className: 'bg-red-50 text-red-700 border-red-200' };
    case 'setup_required':
      return { label: oc?.setupRequired || 'Setup Required', className: 'bg-gray-50 text-gray-500 border-gray-200' };
  }
}

function syncHealthToConnectionStatus(health: PlatformSyncStatus['syncStatus']): 'connected' | 'warning' | 'disconnected' | 'inactive' {
  switch (health) {
    case 'healthy': return 'connected';
    case 'warning': return 'warning';
    case 'error': return 'disconnected';
    case 'setup_required': return 'inactive';
  }
}
...
function PlatformSyncCard({ status, oc }: { status: PlatformSyncStatus; oc: any }) {
  const [expanded, setExpanded] = useState(status.rejected > 0);
  const meta = getPlatformMeta(status.platform);
  const badge = getSyncBadge(status.syncStatus, oc);

  if (status.syncStatus === 'setup_required') {
    return (
      <div className="bg-white rounded-lg border border-dashed border-gray-200 shadow-sm overflow-hidden">
        <div className="p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              <div className={`w-5 h-5 rounded bg-gray-300 flex items-center justify-center text-white text-xs font-bold`}>
                {meta.letter}
              </div>
              <span className="text-sm font-semibold text-gray-400">{meta.name}</span>
            </div>
          ...
          <div className="flex flex-col items-center justify-center py-6 text-center">
            ...
            <p className="text-sm font-medium text-gray-700">
              {(oc?.connectPlatform || 'Connect {{platform}}').replace('{{platform}}', meta.name)}
            </p>
            <p className="text-xs text-gray-500 mt-1 max-w-[200px]">
              {(oc?.connectPlatformDesc || 'Connect your {{platform}} account to start syncing local inventory.').replace('{{platform}}', meta.name)}
            </p>
            <button className="mt-4 px-4 py-2 text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg transition-colors">
              {oc?.startSetup || 'Start Setup'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500 uppercase tracking-wider font-medium">{oc?.published || 'Published'}</div>
          </div>
          <div className="text-center">
            <div className="flex items-center justify-center gap-1 mb-0.5">
            ...
            <div className="text-xs text-gray-500 uppercase tracking-wider font-medium">{oc?.rejected || 'Rejected'}</div>
          </div>
          <div className="text-center">
            <div className="flex items-center justify-center gap-1 mb-0.5">
            ...
            <div className="text-xs text-gray-500 uppercase tracking-wider font-medium">{oc?.pending || 'Pending'}</div>
          </div>
        </div>

        <div className="mb-3">
          <div className="flex items-center justify-between text-xs mb-1">
            <span className="text-gray-500">{oc?.successRate || 'Success rate'}</span>
        ...
        <div className="text-xs text-gray-500">
          {oc?.lastSync || 'Last sync'}: {formatTimeAgo(status.lastSyncAt)}
        </div>
      </div>
        ...
        <>
          ...
          >
            <span className="text-xs font-medium text-red-700">
              {status.rejectionReasons.length} {status.rejectionReasons.length !== 1 ? (oc?.rejectionReasons || 'rejection reasons') : (oc?.rejectionReason || 'rejection reason')}
            </span>
            {expanded
    ...
    <div className="vx-card shadow-sm">
      <div className="vx-card-header">
        <div className="flex items-center gap-1.5">
          <h3 className="text-base font-semibold text-foreground">{oc?.dataConnectionStatus || 'Data Connection Status'}</h3>
          <div className="relative group">
            ...
            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 w-72 z-[9999]">
              {oc?.dataConnectionTooltip || 'End-to-end view of local inventory data — from client ERP into VenueX, then out to Google and Meta.'}
      ...
      <div className="vx-card-body space-y-5">
        ...
        <div className="vx-surface-muted rounded-lg overflow-hidden">
          ...
          <div className="flex items-center justify-between p-4">
            ...
            <div className="flex items-center gap-3 flex-shrink-0 z-10">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900">{oc?.erpSource || 'ERP / Inventory Data'}</p>
                <p className="text-xs text-gray-500">
                  {ingestion.connected ? `${oc?.synced || 'Synced'} ${formatTimeAgo(ingestion.lastReceivedAt)}` : (oc?.notConnected || 'Not connected')}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 flex-shrink-0 z-10">
              <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full border border-gray-200 shadow-sm">
                ...
                <span className="text-xs font-medium text-gray-600">Google Merchant Center</span>
              </div>
              <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full border border-gray-200 shadow-sm">
          ...
          <div className="px-4 pb-4 pt-3 border-t border-gray-200/60">
            <div className="flex items-center gap-2 mb-3">
              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{oc?.lastIngestion || 'Last Ingestion'}</h4>
              <span className={`px-2 py-0.5 text-xs font-medium rounded-full border ${
                ingestion.connected
                  ? 'bg-green-50 text-green-700 border-green-100'
                  : 'bg-red-50 text-red-700 border-red-100'
              }`}>
                {ingestion.connected ? (oc?.success || 'Success') : (oc?.failed || 'Failed')}
              </span>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-xs text-gray-500 mb-1">{oc?.ingestionDate || 'Ingestion Date'}</p>
              ...
              <div>
                <p className="text-xs text-gray-500 mb-1">{oc?.availabilityRecords || 'Availability Records'}</p>
                <p className="text-sm font-medium text-gray-900">{ingestion.itemCount.toLocaleString('tr-TR')}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">{oc?.status || 'Status'}</p>
                <div className={`flex items-center gap-1.5 ${ingestion.connected ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <span className="text-sm font-medium">{ingestion.connected ? (oc?.completed || 'Completed') : (oc?.failed || 'Failed')}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="pt-4 border-t border-gray-200">
          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">{oc?.platformUploadDetails || 'Platform Upload Details'}</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {statuses.map((status) => (
              <PlatformSyncCard key={status.platform} status={status} oc={oc} />
            ))}
          </div>
        </div>
      </div>
